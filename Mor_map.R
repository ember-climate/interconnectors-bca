map_dat <- map_data("world") %>% 
  filter(region %in% neighbours[["Morocco"]]) %>% 
...
i <- filter(intcon, Map=="Morocco")
c <- filter(coal, Country %in% neighbours[["Morocco"]], Status_mod %in% c("Operating", "Planned"))
...
p <- ggplot() +
  ...
  # Capacity bubbles
  # Existing
  geom_point(data=filter(i, Status=="Existing"), aes(x=Cap_lon, y=Cap_lat, stroke=sqrt(cap_e)), shape=21, fill="white", colour="grey20", size=9) +
  ...
  geom_point(data=filter(i, Status=="Existing", cap_p > 0), aes(x=Cap_lon_p, y=Cap_lat_p, stroke=cap_p), shape=21, size=9, colour="firebrick1", fill="white") +
  ...
  # New
  geom_point(data=filter(i, Status=="Planned"), aes(x=Cap_lon_p, y=Cap_lat_p, stroke=sqrt(cap_p)), shape=21, fill="white", colour="firebrick1", size=9) +
